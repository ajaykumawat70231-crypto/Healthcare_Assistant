...
def upload_report():
    if request.method == 'POST':
        ...
        disease_type = request.form['disease_type']  # Assuming this is included in the form
        
        if file:
            # Save the file to the uploads folder
            file.save(os.path.join(app.config['UPLOAD_FOLDER'],disease_type, file.filename))
            # Redirect to the view_report page with both the disease type and filename
            return redirect(url_for('view_report', disease_type=disease_type, report_image=file.filename))
        else:
            return render_template('upload_report.html',error="Please Upload Report!")
    
    return render_template('upload_report.html',error="")
...
def view_report():
        ...
        if(disease_type == "kidney"):
            try:
                ...
                if(ckd_data == "the image is not correct"):
                    return render_template('upload_report.html',error="Please provide correct Report!")
                
            except Exception as e:
                return render_template('upload_report.html',error="Error extracting CKD data: {e}")
                ckd_data = {}
            return render_template('view_report.html', disease_type=disease_type, 
                                report_image=report_image, ckd_data=ckd_data)
            
        elif (disease_type == "lung"):
            model = load_model(os.path.join(app.config['MODELS'],"lung_disease","model.h5"))
    ...
    return render_template('view_report.html', disease_type=disease_type, report_image=report_image)
...
def predict():
    ...
    if disease_type == "kidney":
        ...
        import joblib
        rf_model = joblib.load("static/models/chronic_kidney_disease/Random_Forest_model.pkl")
        ...
        # Save as HTML
        shap.save_html("static/images/force_plot.html", force_plot)
